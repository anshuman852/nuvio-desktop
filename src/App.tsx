...
function Sidebar({ collapsed }: { collapsed: boolean }) {
  ...
  const { setProfileSelected } = useAppStore();
  ...
  return (
    <aside className={clsx(
        ...
        {NAV.map(({ to, icon: Icon, label, exact }) => (
          <NavLink key={to} to={to} end={exact}
            className={({ isActive }) => clsx(
              ...
              isActive ? '' : 'text-white/50 hover:text-white hover:bg-white/5',
      ...
      {/* Profilo attivo — clicca per cambiare */}
      {!collapsed && (
        <button
          onClick={() => setProfileSelected(false)}
          className="flex items-center gap-2 px-4 py-3 border-t border-white/[0.06] hover:bg-white/5 transition-colors text-left"
        >
          <div className="w-7 h-7 rounded-lg flex items-center justify-center text-base flex-shrink-0 text-xl"
...
export default function App() {
  const { profileSelected } = useAppStore();
