...
function LayoutPage() {
  ...
  const T = ({ k, title, desc }: { k: string; title: string; desc: string }) => (
    <Row title={title} subtitle={desc}><Toggle value={!!s[k]} onChange={v => updateSettings({ [k]: v } as any)} /></Row>
  );
  return (
    <div className="space-y-3">
...
function RiproducibilePage() {
  ...
  return (
    <div className="space-y-3">
      ...
      <T k="skipIntro" title="Salta intro" desc="Salta automaticamente intro e riassunti" />
      <Row title="Qualità preferita" subtitle="Qualità default degli stream">
...
const MENU: { id: SubPage; icon: React.ReactNode; label: string }[] = [
  ...
  { id: 'profiles', icon: <Users size={18} />, label: 'Profili' },
  { id: 'aspetto', icon: <Palette size={18}/>, label: 'Aspetto' },
  { id: 'layout', icon: <Grid size={18} />, label: 'Layout' },
  { id: 'integrazioni', icon: <Link2 size={18} />, label: 'Integrazioni' },
  { id: 'riproduzione', icon: <Play size={18} />, label: 'Riproduzione' },
  { id: 'trakt', icon: <Star size={18} />, label: 'Trakt' },
  { id: null, icon: <Info size={18} />, label: 'Informazioni' },
];
...
export default function Settings() {
  ...
  const TITLES: Record<string, string> = { account: 'Account', profiles: 'Profili', aspetto: 'Aspetto', layout: 'Layout', integrazioni: 'Integrazioni', riproduzione: 'Riproduzione', trakt: 'Trakt' };
  const SUBTITLES: Record<string, string> = { account: 'Gestisci il tuo account Nuvio e il sync.', profiles: 'Crea e modifica i profili utente.', aspetto: 'Personalizza colori e tema.', layout: 'Struttura della home e stili.', integrazioni: 'Impostazioni TMDB e MDBList.', riproduzione: 'Player, sottotitoli e riproduzione.', trakt: 'Collega Trakt e Simkl.' };
      ...
      <div className="w-64 flex-shrink-0 overflow-y-auto px-4 py-6 space-y-1.5 border-r border-white/[0.05]">
        ...
        <div className="pt-4 border-t border-white/[0.06] mt-4">
          <button onClick={() => { if (confirm('Cancellare la cronologia locale?')) clearHistory(); }}
            ...
            <Trash2 size={18} className="text-red-400/50" /><span className="text-sm font-medium">Cancella cronologia</span>
          </button>
        </div>
      </div>
