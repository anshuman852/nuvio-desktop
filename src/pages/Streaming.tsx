...
function ServiceLogo({ service, size = 'md' }: { service: StreamingService; size?: 'sm' | 'md' | 'lg' }) {
  const [err, setErr] = useState(false);
  const s = { sm: 'h-6', md: 'h-8', lg: 'h-12' }[size];
  if (!err && service.logo) {
    return <img src={service.logo} alt={service.name} className={`${s} object-contain`} onError={() => setErr(true)} />;
  }
  return <span className="text-2xl">{service.logoFallback}</span>;
}
...
function ContentCard({ item }: { item: ReturnType<typeof tmdbToMeta> }) {
  ...
  return (
    <Link to={`/detail/${item.type}/${encodeURIComponent(item.id)}`} className="flex-shrink-0 group">
      <div className="relative w-[130px] h-[195px] rounded-xl overflow-hidden bg-white/5 border border-white/[0.06] group-hover:border-white/20 group-hover:scale-[1.04] transition-all duration-200 shadow-lg">
        {item.poster && !err
          ? <img src={item.poster} alt={item.name} className="w-full h-full object-cover" onError={() => setErr(true)} />
          : <div className="w-full h-full flex items-center justify-center text-4xl">🎬</div>}
        {item.imdbRating && (
          <div className="absolute top-2 right-2 bg-black/70 backdrop-blur-sm px-1.5 py-0.5 rounded-full text-xs font-bold text-yellow-400">
            ★ {item.imdbRating}
          </div>
        )}
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
      </div>
      <p className="mt-1.5 text-xs text-white/70 group-hover:text-white truncate w-[130px]">{item.name}</p>
...
function ServiceDetail({ service }: { service: StreamingService }) {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      {/* Hero header */}
      <div className="relative flex-shrink-0 min-h-[180px] flex flex-col justify-end px-6 pb-5"
        ...
        <button onClick={() => navigate('/streaming')}
          className="absolute top-5 left-5 flex items-center gap-1.5 text-white/60 hover:text-white text-sm transition-colors z-10">
          <ArrowLeft size={16} />Tutti i servizi
        </button>

        <div className="relative z-10 flex items-end gap-6 mt-10">
          {/* Logo grande */}
          <div className="w-20 h-20 rounded-2xl overflow-hidden flex items-center justify-center bg-black/40 backdrop-blur-sm border border-white/10 flex-shrink-0">
        ...
        <div className="relative z-10 flex items-center gap-3 mt-5">
          ...
          <div className="relative flex-1 max-w-xs">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)}
              ...
              className="w-full pl-8 pr-8 py-2 bg-white/10 backdrop-blur-sm border border-white/10 focus:border-white/30 rounded-full text-sm text-white placeholder:text-white/40 focus:outline-none" />
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5">
        {!hasTMDBKey() ? (
          <div className="flex items-center gap-2 text-yellow-400/80 text-sm bg-yellow-500/10 border border-yellow-500/20 rounded-xl px-4 py-3">
            <AlertCircle size={15} />Aggiungi la chiave API TMDB nelle Impostazioni → Preferenze.
          </div>
    ...
    <div className="px-6 py-6 overflow-y-auto h-full">
      ...
      {!hasTMDBKey() && (
        <div className="flex items-center gap-2 text-yellow-400/80 text-sm bg-yellow-500/10 border border-yellow-500/20 rounded-xl px-4 py-3 mb-6">
          <AlertCircle size={15} />
          <span>Configura la chiave API TMDB nelle <Link to="/settings" className="underline hover:text-yellow-300">Impostazioni</Link>.</span>
        </div>
      )}
