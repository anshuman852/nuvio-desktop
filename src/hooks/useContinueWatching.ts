  ...
  async function load() {
    ...
    if (traktAuth?.token) {
      try {
        ...
        for (const h of history) {
          ...
          seen.set(key, {
            id: imdb ?? `trakt:${h.id}`, type: isMovie ? 'movie' : 'series',
            name: item.title, season: h.episode?.season, episode: h.episode?.number,
            source: 'trakt', watchedAt: new Date(h.watched_at).getTime(),
          });
          if (seen.size >= 20) break;
        }
        setItems(Array.from(seen.values())); setSource('trakt'); setLoading(false); return;
      } catch { /* fallback */ }
    }
    ...
    if (simklAuth?.token) {
      try {
        ...
        const seen = new Set<string>(); const mapped: CWItem[] = [];
        for (const h of history) {
          ...
          mapped.push({ id, type: isMovie ? 'movie' : 'series', name: item.title, source: 'simkl', watchedAt: new Date(h.last_watched_at ?? 0).getTime() });
          if (mapped.length >= 20) break;
        }
        setItems(mapped); setSource('simkl'); setLoading(false); return;
      } catch { /* fallback */ }
    }
    ...
    if (nuvioUser?.token) {
      try {
        ...
        setItems(cw.map((i): CWItem => ({
          ...
          progress: i.progress, source: 'nuvio', watchedAt: new Date(i.updatedAt).getTime(),
        })));
        setSource('nuvio'); setLoading(false); return;
      } catch { /* fallback */ }
    }
    ...
    setSource('local'); setLoading(false);
  }
